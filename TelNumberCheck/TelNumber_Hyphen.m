...
//  TelNumberCheck
//
//  Created by KentarOu on 2014/05/14.
//  Copyright (c) 2014年 KentarOu. All rights reserved.
//

#import "TelNumber_Hyphen.h"

@implementation TelNumber_Hyphen
{
    NSString *telNumber;
}


- (NSString*)telNumber_Hyphen:(NSString*)number
{
    telNumber = number;
    ...
    // 電話番号データ作成
    NSMutableArray *telData = [self makeDataArray];
    return [self validateNumberCheck:telData telNumber:telNumber];
}
...
- (NSString*)validateNumberCheck:(NSMutableArray*)dataArray telNumber:(NSString*)number
{
        ...
        for (NSString *num in array) {
            ...
            for (int i = 2; i < 6; i++) {
                ...
                if ([num isEqualToString:[number substringToIndex:i]]) {
                    return [self hyphenNumberMake:arrayCount];
                }
            }
        }
        arrayCount++;
...
- (NSString*)hyphenNumberMake:(NSInteger)telHyphen_Code
{
    NSMutableString *numStr = [telNumber mutableCopy];
    
    switch (telHyphen_Code) {
        case 0:
            if ([telNumber length] == 10) {
                [numStr insertString:@"-" atIndex:5];
                [numStr insertString:@"-" atIndex:7];
                return numStr;
            } else {
                return telNumber;
            }
